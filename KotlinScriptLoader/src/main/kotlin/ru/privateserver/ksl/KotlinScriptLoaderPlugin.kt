...
class KotlinScriptLoaderPlugin : JavaPlugin() {
    ...
    private fun generateAutocompleteStub() {
        ...
        runCatching {
            ...
            stub.writeText(buildString {
                ...
                appendLine("fun registerCommand(name: String, aliases: List<String> = emptyList(), action: (Player, Array<String>) -> Unit) = Unit")
                appendLine()
    ...
    fun unloadAllScripts() {
        ...
        }.onFailure { logger.warning("Не удалось отменить часть тасков: ${it.message}") }
        tasksByScript.clear()

        runCatching {
            listenersByScript.values.flatten().forEach { HandlerList.unregisterAll(it) }
        }.onFailure { logger.warning("Не удалось снять часть listener'ов: ${it.message}") }
        listenersByScript.clear()

        runCatching { unregisterScriptCommands() }
            .onFailure { logger.warning("Не удалось снять часть команд: ${it.message}") }
        commandsByScript.clear()

        runCatching {
            placeholdersByScript.values.flatten().forEach { placeholders.remove(it) }
        }.onFailure { logger.warning("Не удалось снять часть плейсхолдеров: ${it.message}") }
        placeholdersByScript.clear()

        runCatching {
            loadedScripts.forEach { addonManager.notifyContextDestroyed(it) }
        }.onFailure { logger.warning("Ошибка при уведомлении аддонов о выгрузке: ${it.message}") }
    ...
    private fun unregisterScriptCommands() {
        val ourCommands = commandsByScript.values.flatten().toSet()
        if (ourCommands.isEmpty()) return
        val viaApi = runCatching { removeOurCommands(Bukkit.getCommandMap().knownCommands, ourCommands) }
        if (viaApi.isSuccess) return
        val viaReflection = runCatching { removeOurCommands(reflectKnownCommands(), ourCommands) }
